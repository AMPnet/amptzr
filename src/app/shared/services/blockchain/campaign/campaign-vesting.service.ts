...
export class CampaignVestingService {
  factoryContract$: Observable<CfManagerSoftcapVestingFactory> =
    this.sessionQuery.provider$.pipe(
      map((provider) =>
        CfManagerSoftcapVestingFactory__factory.connect(
          this.preferenceQuery.network.tokenizerConfig.cfManagerFactory.vesting,
          provider
        )
      )
    )
  ...
  ): Observable<CampaignVestingState | undefined> {
          ...
          map(
            (receipt) =>
              findLog(
                receipt,
                contract,
                contract.interface.getEvent('CfManagerSoftcapVestingCreated')
              )?.args?.cfManager
          )
        )
      })
