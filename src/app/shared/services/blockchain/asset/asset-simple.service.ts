...
export class AssetSimpleService {
  factoryContract$: Observable<AssetSimpleFactory> = this.sessionQuery.provider$.pipe(
    map(provider =>
      AssetSimpleFactory__factory.connect(
        this.preferenceQuery.network.tokenizerConfig.assetFactory.simple, provider,
      )),
  )
    ...
    return combineLatest([
      ...
      switchMap(([contract, overrides]) => {
        ...
        return from(contract.populateTransaction.create({
          ...
          map(receipt => findLog(
            receipt, contract, contract.interface.getEvent('AssetSimpleCreated'),
          )?.args?.asset),
        )
      }),
    )
  ...
  getState(address: string, signerOrProvider: Signer | Provider): Observable<SimpleAssetState> {
