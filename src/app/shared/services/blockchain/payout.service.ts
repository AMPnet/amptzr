...
import {PayoutService as ContractPayoutService, PayoutService__factory} from '../../../../../types/ethers-contracts'
...
export class PayoutService {
  ...
  contract(signerOrProvider: Signer | Provider): ContractPayoutService {
    return PayoutService__factory.connect(
      this.preferenceQuery.network.tokenizerConfig.payoutService, signerOrProvider,
    )
  }
  ...
  getPayoutFeeForAssetAndAmount(asset: string, amount: BigNumber) {
    return this.sessionQuery.provider$.pipe(
      ...
      switchMap(contract => contract.getPayoutFeeForAssetAndAmount(
        asset, amount,
