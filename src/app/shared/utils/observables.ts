...
export function withInterval<T>(
  ...
  return timer(0, offset).pipe(switchMap(() => observable$))
}
...
export function withStatus<T>(
  ...
  return new Observable((subscriber) => {
    const subscription = refreshSub
      .asObservable()
      .pipe(switchMap(() => concat(...stream)))
      .subscribe({
...
export function switchMapTap<T>(
  next: (value: T) => Observable<unknown>
): MonoTypeOperatorFunction<T> {
  return switchMap((source) => next(source).pipe(map(() => source)))
}
