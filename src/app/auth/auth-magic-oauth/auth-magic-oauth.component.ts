...
export class AuthMagicOauthComponent {
  ...
  constructor(private route: ActivatedRoute,
    ...
    this.handleCallback1$ = defer(() => of(this.route.snapshot.queryParams)).pipe(
    ...
    this.handleCallback2$ = defer(() => of(this.route.snapshot.queryParams)).pipe(
      filter(params => !!params.redirectBack),
      switchMap(params => withStatus(
        this.magicSubsignerService.registerMagic.pipe(
          ...
          concatMap(() => this.router.router.navigateByUrl(params.redirectBack)),
        ),
      )),
    )
  }
