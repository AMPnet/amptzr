...
import {combineLatest, concatMap, from, Observable, of, switchMap, tap} from 'rxjs'
import {SessionQuery} from '../../../session/state/session.query'
import {catchError, debounceTime, distinctUntilChanged, map, shareReplay, startWith, take} from 'rxjs/operators'
...
export class SnapshotNewComponent {
  ...
  constructor(private payoutService: PayoutService,
    ...
    const blockNumber$: Observable<string> = this.newSnapshotForm.get('blockNumber')!.valueChanges.pipe(
  ...
  private assetValidator(_control: AbstractControl): Observable<ValidationErrors | null> {
  ...
  private blockNumberValidator(_control: AbstractControl): Observable<ValidationErrors | null> {
    return combineLatest([this.state$]).pipe(
      debounceTime(100),
      take(1),
