...
export class HomeComponent implements OnInit {
  featuredMappings: { [key in ChainID]?: string[] } = {
    [ChainID.MATIC_MAINNET]: [
      '0x950c8472c5bb8a10e111646a3de7fd226306b5cf',
    ],
    [ChainID.MUMBAI_TESTNET]: [
      '0x5d1800560a29ba3e8130D2B48Bd4c510a506DD62',
    ],
  }
  ...
  featuredIssuers$: Observable<WithStatus<IssuerItem[]>> = this.featuredAddrs$.pipe(
    switchMap(issuersAddrs => withStatus(
        issuersAddrs.length === 0 ? of([]) : combineLatest(
          issuersAddrs.map(address => this.queryService.getIssuerForAddress(address).pipe(
            switchMap(issuer => this.issuerService.getIssuerInfo(issuer.issuer).pipe(
  ...
  openIssuer(issuer: IssuerItem) {
    return this.router.navigateIssuer(issuer.mappedName || issuer.issuer.contractAddress)
  }
