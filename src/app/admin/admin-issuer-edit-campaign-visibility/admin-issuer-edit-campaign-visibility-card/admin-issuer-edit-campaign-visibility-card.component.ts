...
export class AdminIssuerEditCampaignVisibilityCardComponent implements OnChanges, OnInit {
  ...
  campaign$: Observable<WithStatus<CampaignCommonStateWithName & CampaignWithInfo>> = this.campaignSub.asObservable()
    .pipe(
  ...
  ngOnInit() {
    this.campaignVisibilityChange.next({campaignAddress: this.offer.campaign.contractAddress, isHidden: this.isHidden})
  }

  ngOnChanges(changes: SimpleChanges) {
    setTimeout(() => {
      this.campaignSub.next(changes.offer.currentValue as CampaignCommonStateWithName)
    })
  }
