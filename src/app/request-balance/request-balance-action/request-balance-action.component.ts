...
import {RequestBalance, RequestBalanceService} from '../request-balance.service'
  ...
  confirmOwnership(state: RequestBalanceState) {
    return () => {
      return this.signerService.ensureAuth.pipe(
        switchMap(() => this.signerService.signMessage(state.requestBalance.message_to_sign)),
        switchMapTap(signedMessage => this.requestBalanceService.updateRequest(state.requestBalance.id, {
